                            ...
                            <?php if (!empty($list)): foreach ($list as $first): ?>
                                <tr>
                                    ...
                                    <td class="am-text-middle">
                                           ...
                                           am-badge-<?= $first['status'] == 10 ? 'success' : 'warning' ?>"
                                                 data-id="<?= $first['category_id'] ?>"
                                                 data-state="<?= $first['status'] ?>">
                                               <?= $first['status'] == 10 ? '显示' : '隐藏' ?>
                                           </span>
                                    </td>
                                ...
                                <?php if (isset($first['child'])): foreach ($first['child'] as $two): ?>
                                    <tr>
                                        ...
                                        <td class="am-text-middle">
                                               <span class="j-state am-badge x-cur-p
                                               am-badge-<?= $two['status'] == 10 ? 'success' : 'warning' ?>"
                                                     data-id="<?= $two['category_id'] ?>"
                                                     data-state="<?= $two['status'] ?>">
                                                   <?= $two['status'] == 10 ? '显示' : '隐藏' ?>
                                               </span>
                                        </td>
                                    ...
                                    <?php if (isset($two['child'])): foreach ($two['child'] as $three): ?>
                                        <tr>
                                            ...
                                            <td class="am-text-middle">
                                                   <span class="j-state am-badge x-cur-p
                                                   am-badge-<?= $three['status'] == 10 ? 'success' : 'warning' ?>"
                                                         data-id="<?= $three['category_id'] ?>"
                                                         data-state="<?= $three['status'] ?>">
                                                       <?= $three['status'] == 10 ? '显示' : '隐藏' ?>
                                                   </span>
                                            </td>
                                            <td class="am-text-middle">
                                                <div class="tpl-table-black-operation">
...
<script>
    $(function () {
        ...
        $('.j-state').click(function () {
            var data = $(this).data();
            layer.confirm('确定要' + (parseInt(data.state) === 10 ? '隐藏' : '显示') + '该分类吗？'
                , {title: '友情提示'}
                , function (index) {
                    $.post("<?= url('goods.category/state') ?>"
                        , {
                            category_id: data.id,
