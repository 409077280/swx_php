...
class Order extends Controller
{
    ...
    public function detail($order_id)
    {
        ...
        foreach($order->goods as &$orderGoods) {
            ...
            $orderGoodsModel = OrderGoodsModel::detail($orderGoods->order_goods_id);
            $isAllowRefund   = $orderGoodsModel->isAllowRefund($order, $goods);
            $orderGoods['isAllowRefund'] = $isAllowRefund;
        }
        // 该订单是否允许申请售后
    ...
    public function pay($order_id, $payType = PayTypeEnum::WECHAT)
    {
        ...
        */
        // 判断是否达成购买条件
        if (!$model->checkGoodsCondition($model['goods'], $this->user)) {
            return $this->renderError($model->getError());
        }
        $render = [
