 ...
 * @return bool|int
 */
function write_log($values, $dir = '', $fileName = '')
{
    if (is_array($values))
    ...
    try {
        // 文件路径
        $dir = $dir ? $dir : RUNTIME_PATH;
        $filePath = $dir . '/log/' . date('Ym') . '/';
        // 路径不存在则创建
        !is_dir($filePath) && mkdir($filePath, 0755, true);
        // 写入文件
        $fileName = $fileName ? $fileName : date('Ymd') . '.log';
        return file_put_contents($filePath . $fileName, $content, FILE_APPEND);
    } catch (\Exception $e) {
        return false;
    }
}
 ...
 * CDN图片处理
 */
 function format_image($imageItems) {
     return $imageItems;
     if($imageItems) {
         if(is_array($imageItems)) {
             foreach($imageItems as &$image) {
                 $image['file_path'] .= '?x-oss-process=image/auto-orient,1/resize,m_lfit,w_800/quality,q_90';
             }
         } else {
             return $imageItems . '?x-oss-process=image/auto-orient,1/resize,m_lfit,w_800/quality,q_90';
         }
     }
     return $imageItems;
 }
  ...
  */
  function format_content($content) {
      if($content) {
          $content = preg_replace_callback('/http:\/\/swxcdn.xihongshijie.com\/(.*?)+(.jpg|.png|.gif|.jpeg|.bmp)/i', function ($matches) {
              // return $matches[0] . '?x-oss-process=image/auto-orient,1/resize,m_lfit,w_800/quality,q_90';
              return $matches[0] . '?x-oss-process=style/swx-image-compress';
          }, $content);
      }
