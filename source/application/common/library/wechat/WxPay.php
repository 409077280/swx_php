...
class WxPay extends WxBase
{
    ...
    // 订单模型
    private $modelClass = [
        OrderTypeEnum::MASTER   => 'app\task\model\Order',
        OrderTypeEnum::SHARING  => 'app\task\model\sharing\Order',
        OrderTypeEnum::RECHARGE => 'app\task\model\recharge\Order',
        OrderTypeEnum::OFFLINE  => 'app\task\model\offline\Order',
    ];
...
//EOF;
        ...
        $this->doLogs($data);
        // 实例化订单模型
        $OrderModel = $this->getOrderModel($data['attach']);
        // 订单信息
        ...
        if (
            ...
            // 订单支付成功业务处理
            $order->paySuccess(PayTypeEnum::WECHAT, $data);
            // 返回状态
