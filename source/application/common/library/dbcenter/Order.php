...
class Order {
    ...
    public function report($order_id, $user_id) {
        ...
        $settingGoodsShare = SettingModel::getItem('goods_share', $orderInfo->wxapp_id);
        if($orderInfo) {
            $goodsItems = [];
            if($orderInfo->goods) {
                foreach($orderInfo->goods as $orderGoods) {
                    ...
                    // 商品推荐人及贡献率
                    if($settingGoodsShare['is_open'] == '1') {
                        $temp['relateUserCode']           = $orderGoods->goods_referee_id;
                        $temp['relateContributionRate']   = floatval($settingGoodsShare['referee']) / 100;
                    } else {
                        $temp['relateUserCode']           = 0;
                        $temp['relateContributionRate']   = 0;
                    }
