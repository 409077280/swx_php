...
class Extract extends Base
{
     ...
     * @param $orderType
     */
    public function __construct($wxappId, $user, $orderId, $orderType = OrderTypeEnum::MASTER, $goods_id = 0, $goods_sku_id = 0)
    {
        parent::__construct();
    ...
    public function getImage()
    {
        ...
        $qrcode = $this->saveQrcode(
            $this->wxappId,
            "oid:{$this->orderId},oty:{$this->orderType},gid:{$this->goodsId},gsid:{$this->goodsSkuId}",
    ...
    {
        return 'extract_' . md5("{$this->orderId}_{$this->goodsId}_{$this->goodsSkuId}_{$this->user['open_id']}}") . '.png';
    }
