...
class Category extends BaseModel
{
    ...
    public static function getALL($status = '')
    {
        ...
        if (true) {
            if($status)
                $data = $model->with(['image'])->where(['status' => $status])->order(['sort' => 'asc', 'create_time' => 'asc'])->select();
            else
     ...
     * @return mixed
     */
    public static function getCacheTree($status = '')
    {
        return self::getALL($status)['tree'];
    }
