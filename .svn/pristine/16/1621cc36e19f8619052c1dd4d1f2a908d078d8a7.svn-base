...
class OrderRefund extends OrderRefundModel
{
    ...
    public function audit($data)
    {
        ...
        try {
            ...
            // 发送模板消息
            (new Message)->refund(static::detail($this['order_refund_id']), $order['order_no'], OrderTypeEnum::SHARING);
            // 事务提交
            $this->commit();
            return true;
        } catch (\Exception $e) {
    ...
    public function receipt($data)
    {
        ...
        $this->transaction(function () use ($order, $data) {
            ...
            // 发送模板消息
            (new Message)->refund(self::detail($this['order_refund_id']), $order['order_no'], OrderTypeEnum::SHARING);
        });
        return true;
    }
