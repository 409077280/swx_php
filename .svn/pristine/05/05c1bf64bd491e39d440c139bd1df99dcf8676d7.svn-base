...
use app\common\library\dbcenter\sharing\Order as DBCenterSharingOrderModel;
...
class Order extends OrderModel
{
    ...
    private function updatePayStatus($payType, $payData)
    {
	    ...
	    // 上传订单数据到数据中心 Added by Jerry @ 2019-03-10
            (new DBCenterSharingOrderModel)->report($this['order_id'], $this['user_id']);
        });
        return true;
    }
    ...
    public function updateFailedStatus($orderList)
    {
        ...
        foreach ($orderList as $order) {
            ...
            try {
                ...
                // 上报数据中心申请退款(扣减贡献和分红)
                (new DBCenterSharingOrderModel)->cancel($order['order_id'], $order['user_id']);
