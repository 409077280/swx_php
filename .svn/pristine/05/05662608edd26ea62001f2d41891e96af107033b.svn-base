...
class Cart extends Controller
{
     ...
     * @param string $tag 商品分享链接标识
     * @return array
     * @throws \app\common\exception\BaseException
     * @throws \think\exception\DbException
     */
    public function add($goods_id, $goods_num, $goods_sku_id, $goods_referee_id = 0, $tag = '')
    {
        // 判断商品分享是否在有效期
        if(!(new ShareModel)->checkGoodsShareExpires('goods', $goods_referee_id, $goods_id, $tag))
            $goods_referee_id = 0;

        if (!$this->model->add($goods_id, $goods_num, $goods_sku_id, $goods_referee_id)) {
     ...
     * @return array
     */
    public function input($goods_id, $goods_num, $goods_sku_id) {
        if (!$this->model->input($this->user, $goods_id, $goods_num, $goods_sku_id)) {
            return $this->renderError($this->model->getError() ?: '加入购物车失败');
        }
        $total_num = $this->model->getTotalNum();
        return $this->renderSuccess(['cart_total_num' => $total_num], '加入购物车成功');
