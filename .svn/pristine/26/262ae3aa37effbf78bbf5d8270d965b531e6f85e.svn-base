...
class Goods extends Base
{
    ...
    private function savePoster($backdrop, $goodsUrl, $qrcode)
    {
        ...
        // 重设商品图片宽高
        $editor->resizeExact($goodsImage, 710, 710);
        // 商品图片添加到背景图
        $editor->blend($backdropImage, $goodsImage, 'normal', 1.0, 'top-left', 20, 185);
        // 商品名称处理换行
        $fontSize = 30;
        $goodsName    = $this->wrapText($fontSize, 0, $fontPath, $this->goods['goods_name'], 710, 2);
        $sellingPoint = $this->wrapText($fontSize, 0, $fontPath, $this->goods['selling_point'], 650, 2);
        // 写入商品名称
        $editor->text($backdropImage, $goodsName, $fontSize, 20, 920, new Color('#333333'), $fontPath);
        $editor->text($backdropImage, $sellingPoint, 20, 20, 1210, new Color('#FFC748'), $fontPath);
        // 写入商品价格
        $priceType = [10 => 'goods_price', 20 => 'sharing_price'];
        $editor->text($backdropImage, $this->goods['sku'][0][$priceType[$this->goodsType]], 52, 80, 1058, new Color('#da3e44'));
        $editor->text($backdropImage, '———————', 24, 20, 1140, new Color('#aaaaaa'), $fontPath);
        $editor->text($backdropImage, '￥' . $this->goods['sku'][0]['line_price'], 24, 35, 1140, new Color('#aaaaaa'), $fontPath);
        // 打开小程序码
        $editor->open($qrcodeImage, $qrcode);
        // 重设小程序码宽高
        $editor->resizeExact($qrcodeImage, 180, 180);
        // 小程序码添加到背景图
        $editor->blend($backdropImage, $qrcodeImage, 'normal', 1.0, 'top-left', 510, 1070);
        // 保存图片
